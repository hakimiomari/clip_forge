...
2. **New project** → choose **Upload file** (MP4/MOV/MKV/WebM up to 4 GB), tick the rights confirmation, and create. The file goes straight to storage via a presigned URL; the worker probes it, makes a thumbnail, extracts audio, and marks the project **Imported** — progress streams live.
   - *YouTube URL* imports metadata in seconds (max 4 hours). Highlight analysis downloads the audio track (and a ≤360p video track for scene detection) into a temp folder with `yt-dlp` and analyses it locally — reading YouTube's stream directly is throttled to about playback speed, which made long videos appear to hang. Each render then fetches only that clip's windows. The temp folder is deleted when the job ends; nothing from YouTube is stored except your rendered clips.
3. The project page offers three ways to get clips — use whichever suits the job:
   - **Automatic shorts** — one button. Pick how many shorts, how long, the format and caption style, then **Make N shorts automatically**: ClipForge finds the best moments *and* renders a finished short for each one. The work runs in the background, so you can close the page. Costs 2 credits to find the moments plus the usual render cost per short; if fewer good moments exist than you asked for, the unused credits are returned automatically.
   - **Suggestions only** — analyse and propose moments without rendering (2 credits), so you can review each one's score, hook and reason and choose which become clips.
   - **The timeline** — cut it yourself (see below).
   Either way, the collapsible **Transcript** panel shows the fetched transcript (YouTube captions are used automatically when available).
4. **Or cut it yourself on the timeline.** The project page opens with a scrubbable timeline under the player, showing real frames from the video. The playhead and the shaded fill track playback live, so the timeline doubles as a progress bar. Drag the blue handles (or press <kbd>I</kbd>/<kbd>O</kbd> to set start/end at the playhead), drag the middle to slide the window, type exact seconds, and **Preview selection** to watch just that part. <kbd>Space</kbd> plays, arrows step a second (hold <kbd>Shift</kbd> for 0.1 s); zoom in for frame-level trimming on long videos, and **Fit selection** to zoom straight to your window. AI suggestions appear as markers on the track — click one to load its range. Then **Create clip from selection** (2 credits per 30 s, 5–240 s per clip). The frames are built once per project in the background; on YouTube sources they are sampled from the stream, so nothing is downloaded.
   - **One clip from several moments:** set a selection and press **Add as part**, then repeat for each moment you want (up to 12). The parts list shows them in order — reorder with the arrows, **Show** to jump back to one, or remove it — and the track marks each part. **Create clip from N parts** stitches them together in order into a single short, with captions, effects and the banner applied across the whole thing. For YouTube only those windows are fetched, never the full video.
5. Each suggestion card shows a **score /100, title, time range, hook and the reason** it was selected. Set the **Clip options** row first — captions on/off, slow zoom, background mode (blurred bars / fill screen / black bars), Like & Follow banner — then click **Create clip** (2 credits per 30 s).
6. On the clip card: watch render progress, then **preview**, **Download MP4**, **Re-render**, **Delete**, or open the two editors:
...
7. Click **your name** at the bottom of the sidebar for settings: profile, password change, and your full credit history.
