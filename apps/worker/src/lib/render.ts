...
export interface RenderSpec {
  ...
  threads?: number;
  /**
   * AI subject cut-out: raw gray8 320×320@30 mask video generated by
   * lib/bg-removal. Replaces the normal layout; incompatible with speed
   * effects (stripped upstream).
   */
  backgroundRemoval?: {
    maskPath: string;
    replace: "blur" | "color";
...
export function buildFilterGraph(spec: RenderSpec): string {
  ...
  // ── Layout ─────────────────────────────────────────────
  if (spec.backgroundRemoval) {
    const hasTrailInput = (spec.rangeEffects ?? []).some(
      (e) => e.type === "glow_trail",
    ) && spec.glowSpritePath;
    ...
    if (spec.backgroundRemoval.replace === "color") {
      const hex = (spec.backgroundRemoval.color ?? "0b0d12").replace(/[^0-9a-fA-F]/g, "").slice(0, 6);
      chains.push(`color=c=0x${hex || "0b0d12"}:s=${w}x${h}:r=${fps}[bg]`);
    } else {
      const qw = Math.round(w / 4 / 2) * 2;
      const qh = Math.round(h / 4 / 2) * 2;
      chains.push(
        `[brb]scale=${qw}:${qh}:force_original_aspect_ratio=increase,crop=${qw}:${qh},boxblur=luma_radius=7:luma_power=2,eq=brightness=-0.08,scale=${w}:${h}[bg]`,
      );
    }
    chains.push(
      `[cut]scale=${w}:${h}:force_original_aspect_ratio=decrease[cutf]`,
      `[bg][cutf]overlay=(W-w)/2:(H-h)/2:shortest=1[vbase]`,
    );
  } else if (spec.fillFrame) {
  ...
  if (spec.watermarkText) {
    ...
    tail += `,drawtext=text='${spec.watermarkText.replace(/[':\\]/g, "")}':font='Arial':fontcolor=white:alpha=0.55:fontsize=${size}:x=(w-tw)/2:y=${pad}`;
  }
...
export function buildRenderArgs(spec: RenderSpec): string[] {
  ...
  if (spec.backgroundRemoval) {
    args.push(
      ...
      "-video_size", "320x320",
      "-framerate", "30",
      "-i", spec.backgroundRemoval.maskPath,
    );
  }
  args.push(
